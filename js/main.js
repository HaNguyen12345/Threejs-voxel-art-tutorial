...
function createMainScene() {
  ...
  topLight = new THREE.SpotLight(0xffffff, 0.4);
  ...
  voxelGeometry = new RoundedBoxGeometry(
    params.boxSize,
    params.boxSize,
    params.boxSize,
    2,
...
function animateVoxels(oldModelIdx, newModelIdx) {
  ...
  for (let i = 0; i < voxels.length; i++) {
    ...
    if (voxelsPerModel[newModelIdx][i]) {
      ...
      targetPos =
        voxelsPerModel[newModelIdx][
          Math.floor(voxelsPerModel[newModelIdx].length * Math.random())
        ].position;
    }
...
function updateSceneSize() {
  ...
  previewScenes.forEach((scene) => {
    scene.userData.element.style.width =
      Math.min(90, (window.innerHeight * 0.8) / modelURLs.length) + "px";
  });
  previewScenes.forEach((scene) => {
    ...
    scene.userData.camera.aspect =
      scene.userData.element.clientWidth / scene.userData.element.clientHeight;
...
function createControls() {
  ...
  gui
    .add(params, "boxSize", 0.05, params.gridSize)
    .step(0.01)
    .onChange(() => {
      instancedMesh.geometry = new RoundedBoxGeometry(
        params.boxSize,
        params.boxSize,
        params.boxSize,
        2,
    ...
    .name("voxel size");
  gui
    .add(params, "boxRoundness", 0, 0.5 * params.gridSize)
    .step(0.01)
    .onChange(() => {
      instancedMesh.geometry = new RoundedBoxGeometry(
        params.boxSize,
